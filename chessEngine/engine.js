  ...
  function addMove(moveList, move) {
    ...
    if (getMoveCapture(move)) {
      ...
      if (killerMoves[searchPly] == move) moveScore = 9000
        ...
        moveScore =
          historyMoves[board[getMoveSource(move)] * 128 + getMoveTarget(move)]
    }
          ...
          if (pieceType == PAWN) {
            ...
            if ((targetSquare & 0x88) == 0 && board[targetSquare] == e) {
              if ((targetSquare & 0xf0) == pawnPromotingRank[side]) {
                for (
                  ...
                  addMove(
                    ...
                    encodeMove(
                      sourceSquare,
                      targetSquare,
                      mapFromOptimized[promotedPiece | (side << 3)],
                      0,
                      0,
                      0,
                      0
                ...
                if (
                  (sourceSquare & 0xf0) == pawnStartingRank[side] &&
  ...
  */
  ...
  function getGamePhaseScore() {
    ...
    for (let piece = N; piece <= Q; piece++)
      gamePhaseScore += pieceList[piece] * materialWeights[opening][piece]
    for (let piece = n; piece <= q; piece++)
      gamePhaseScore += pieceList[piece] * -materialWeights[opening][piece]
  ...
  function searchPosition(depth) {
    ...
    for (let currentDepth = 1; currentDepth <= depth; currentDepth++) {
      ...
      if (typeof document != "undefined") {
        ...
        let guiScore = uciScore
        let guiDepth = info.split("depth ")[1].split(" ")[0]
        let guiPv = info.split("pv ")[1]
        let guiTime = info.split("time ")[1].split(" ")[0]
      }
  ...
  function printBoard() {
    ...
    boardString +=
      "\n   moves:          " +
      (gamePly % 2 ? Math.round(gamePly / 2) - 1 : Math.round(gamePly / 2))
  ...
  function moveToString(move) {
    if (getMovePromoted(move)) {
      ...
      return coordinates[getMoveSource(move)] + coordinates[getMoveTarget(move)]
    }
  }
